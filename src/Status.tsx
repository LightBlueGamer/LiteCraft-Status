...
for (const s of servs) {
  ...
  const obj = {
    name: s.name,
    status: data.online ? "Online" : "Offline" as const,
...
const Status: Component = () => (
  <div class='bg-slate-900 inset-0 h-full w-full z-10 flex flex-row place-content-center place-items-center'>
    <div class='bg-blue-800 flex grow flex-col place-content-center gap-4 max-w-sm p-4 rounded mx-2'>
      <h1 class='text-4xl font-bold text-center p-1'>LCN Servers</h1>

      <For each={servers}>
        {(server) => (
          <a
            class='bg-slate-800 border-slate-400 hover:slate-900 flex flex-col gap-2 h-11 place-content-between rounded px-2 font-semibold leading-none focus:ring-2 focus:ring-blue-500'
